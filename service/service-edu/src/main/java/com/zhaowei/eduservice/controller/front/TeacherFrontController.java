...
public class TeacherFrontController {
    ...
    public ResponseInfo getTeacherFrontList(@PathVariable long page, @PathVariable long limit){
        Page<EduTeacher> pageTeacher = new Page<>(page, limit);
        Map<String, Object> map = teacherService.getTeacherFrontList(pageTeacher);

        //返回分页所有数据
        return ResponseInfo.ok().data(map);
    }

    //2 讲师详情的功能
    @GetMapping("getTeacherFrontInfo/{teacherId}")
    public ResponseInfo getTeacherFrontInfo(@PathVariable String teacherId){
        //1 根据讲师id查询讲师基本信息
        ...
        return ResponseInfo.ok().data("teacher", teacher).data("courseList", courseList);
    }
}
